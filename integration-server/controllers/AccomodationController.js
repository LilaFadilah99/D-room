...
class AccomodationController {
  ...
  static async addAccomodation(request, response, next) {
    const { name, imgUrl, description, roomCapacity, price, serviceCharge, location, facility, CategoryId, TypeId, cleaningFee } = request.body;
  ...
  static async deleteAccomodation(request, response, next) {
    ...
    try {
      ...
      response.status(200).json({ message: `succes to delete accomodation id ${id}` });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async getDataFavorites(request, response, next) {
    ...
    try {
      ...
      const favorite = await Favorite.findAll({ where: { UserId: id }, include: [{ model: Accommodation }, { model: User, attributes: ["id", "username", "email", "role"] }], order: [["updatedAt", "DESC"]] });
  ...
  static async deleteDataFavorite(request, response, next) {
    ...
    try {
      ...
      response.status(200).json({ message: `Success delete favorite` });
    } catch (error) {
      next(error);
    }
  }
  ...
  static async handleGetBookingRoom(request, response, next) {
    ...
    console.log(UserId);
    try {
      const bookingRoom = await BookingRoom.findOne({ where: { AccomodationId: id, UserId }, include: [{ model: Accommodation }, { model: User, attributes: ["id", "username", "email", "role"] }], order: [["createdAt", "DESC"]] });
      console.log(bookingRoom);
