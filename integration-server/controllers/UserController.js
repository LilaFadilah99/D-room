...
class UserController {
  ...
  static async updateUser(request, response, next) {
    try {
      ...
      let [user, returnData] = await User.update({ username, email, password: newPassword, role }, { where: { id: id }, returning: ["username", "email", "role"] });
      console.log(user);
  ...
  static async deleteUser(request, response, next) {
    ...
    try {
      ...
      if (!user) throw { message: "Data Not Found", status: 404 };
      response.status(200).json({ message: `succes to delete user id ${id}` });
    } catch (error) {
      console.log(error);
      next(error);
    }
